    ...
    fn getRootLazyPath(b: *Build, selection: Selection) LazyPath {
        inline for (Chapters) |chapter| {
            ...
            if (meta.stringToEnum(chapter, @tagName(selection))) |ex_name| {
                ...
                const fmt = comptimeJoin(.file, .{ "examples", dir, "{d:0>2} {s}.zig" });
...
inline fn comptimeJoin(comptime kind: enum { dir, file }, comptime parts: anytype) []const u8 {
    comptime var str: []const u8 = &.{};
    inline for (parts, 0..) |part, i| {
        const sep = if (i < parts.len - 1) std.fs.path.sep_str else "";
        str = str ++ part ++ sep;
    }
    return str ++ if (kind == .dir) std.fs.path.sep_str else "";
}
